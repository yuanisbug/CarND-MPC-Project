...
The inputs to the model are: state [x, y, ψ, ν] and actuations [δ, a]. MPC uses an optimizer to find the inputs that minimize the cost function. The elements of the cost function are:
...
We pass the current state as the initial, call the optimization solver which then returns vector of the control inputs. The first control inputs are applied and then the solver is called again based on the new state of the car returned by the simulator.
...
**Choose N and dt considerations:**

Larger N would cost time for processing. Smaller dt is better but limited by processing time. N*dt is related to the car speed. Here we have N = 10 and dt = 0.1 s, which is compared to other values in the test.
...
The cost model uses weights and penalizes deviations from the trajectory (CTE), heading errors, use of actuators and large changes in actuations. Setting the weights low, caused the car to veer off the track especially at high speeds. This was essentially a trial and error exercise to find the weights that performed reasonably for the reference velocity of 70mph.

**Accounting for Latency**

Predicting the car position in the latency interval and using that as the initial state
This approach predict the likely car position after time interval 100ms using the same update equations from the vehicle model and then feeding this new predicted state to the solver. The time step dt was set to 0.1s, conveniently chosen to be a multiple of the latency which allowed the control variable to be picked up from one step before.

The details can be seen from the C++ files and simulator.
---
## Dependencies
